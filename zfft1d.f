...
C
      ...
      COMPLEX*16 W1(NDA2/3*2+NP),W2(NDA2/3*2+NP)
...
!$OMP PARALLEL PRIVATE(C,D)
          CALL ZFFT1D0(A,B,C,D,W1,W2,WW1,WW2,WW3,WW4,N1,N2,M1,M2,
     1                 IP1,IP2)
!$OMP END PARALLEL
        ELSE
!$OMP PARALLEL PRIVATE(C,D)
          CALL ZFFTR1D0(A,B,C,D,W1,W2,WW1,WW2,WW3,WW4,N1,N2,M1,M2,
     1                  IP1,IP2)
!$OMP END PARALLEL
        END IF
      END IF
...
!$OMP DO
  ...
  150 CONTINUE
      RETURN
      END
      SUBROUTINE ZFFTR1D0(A,B,C,D,W1,W2,WW1,WW2,WW3,WW4,N1,N2,M1,M2,
     1                    IP1,IP2)
      IMPLICIT REAL*8 (A-H,O-Z)
      INCLUDE 'param.h'
      COMPLEX*16 A(N1,*),B(N2,*),C(N2+NP,*),D(*)
...
!$OMP DO PRIVATE(IJ,IJ0,IR,J,TEMP)
   ...
   30   CONTINUE
        DO 40 I=II,MIN0(II+NBLK-1,N1)
          CALL FFT23458R(C(1,I-II+1),D,W2,N2,IP2)
   40   CONTINUE
        IF (2**IP1(1) .LT. NBLK .OR. 2**IP2(1) .LT. NBLK) THEN
          DO 70 IS=1,N2/M2
            DO 60 IK=1,M2
              ...
              DO 50 I=II,MIN0(II+NBLK-1,N1)
                IR=(I-1)/M1+1
                IJ=MOD(I-1,M1)+1
                A(I,J)=C(J,I-II+1)*DCONJG(WW1(IK,IJ)*WW2(IS,IJ)
     1                *WW3(IK,IR)*WW4(IS,IR))*DN
   50         CONTINUE
   60       CONTINUE
   70     CONTINUE
        ELSE
          ...
          DO 100 IS=1,N2/M2
            DO 90 IK=1,M2
              TEMP=WW3(IK,IR)*WW4(IS,IR)*DN
              J=IK+(IS-1)*M2
              IJ=IJ0
              DO 80 I=II,MIN0(II+NBLK-1,N1)
                A(I,J)=C(J,I-II+1)*DCONJG(WW1(IK,IJ)*WW2(IS,IJ)*TEMP)
                IJ=IJ+1
   80         CONTINUE
   90       CONTINUE
  100     CONTINUE
...
!$OMP DO
      DO 150 JJ=1,N2,NBLK
        DO 120 J=JJ,MIN0(JJ+NBLK-1,N2)
          CALL FFT23458R(A(1,J),C,W1,N1,IP1)
  120   CONTINUE
        DO 140 I=1,N1
          DO 130 J=JJ,MIN0(JJ+NBLK-1,N2)
            B(J,I)=A(I,J)
