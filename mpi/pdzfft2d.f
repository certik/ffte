...
!$OMP PARALLEL PRIVATE(C,D)
      CALL PDZFFT2D0(A,A,A,B,B,B,C,C,D,WX,WY,NX,NY,LNX,LNY,ICOMM,ME,NPU,
     1               IOPT)
!$OMP END PARALLEL
      RETURN
      END
      SUBROUTINE PDZFFT2D0(DA,A,AYX,B,BXY,BYX,CX,CY,D,WX,WY,NX,NY,LNX,
...
!$OMP BARRIER
!$OMP MASTER
      CALL MPI_ALLTOALLV(BYX,ISCNT,ISDSP,MPI_DOUBLE_COMPLEX,AYX,IRCNT,
...
!DIR$ VECTOR ALIGNED
          DO 270 I=1,NNX/2
            BXY(I+(K-1)*(NNX/2)+1,J)
     1     =A(I+(J-1)*(NNX/2)+((K-1)*(NNX/2)+1)*NNY)
  270     CONTINUE
  280   CONTINUE
  290 CONTINUE
      RETURN
