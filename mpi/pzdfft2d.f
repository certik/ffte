...
!$OMP PARALLEL PRIVATE(C,D)
      CALL PZDFFT2D0(A,A,A,B,B,B,C,C,D,WX,WY,NX,NY,LNX,LNY,ICOMM,ME,NPU,
     1               IOPT)
!$OMP END PARALLEL
      RETURN
      END
      SUBROUTINE PZDFFT2D0(A,AXY,AYX,B,BYX,DB,CX,CY,D,WX,WY,NX,NY,LNX,
     1                     LNY,ICOMM,ME,NPU,IOPT)
      IMPLICIT REAL*8 (A-H,O-Z)
      INCLUDE 'mpif.h'
      INCLUDE 'param.h'
...
!DIR$ VECTOR ALIGNED
            DO 50 I=1,NNX/2
              B(I+(J-1)*(NNX/2)+((K-1)*(NNX/2)+1)*NNY)
     1       =AXY(I+(K-1)*(NNX/2)+1,J)
   50       CONTINUE
   60     CONTINUE
   70   CONTINUE
!$OMP BARRIER
...
!$OMP BARRIER
!$OMP MASTER
      CALL MPI_ALLTOALLV(BYX,IRCNT,IRDSP,MPI_DOUBLE_COMPLEX,AYX,ISCNT,
     1                   ISDSP,MPI_DOUBLE_COMPLEX,ICOMM,IERR)
!$OMP END MASTER
!$OMP BARRIER
