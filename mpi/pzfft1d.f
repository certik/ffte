...
!$OMP END PARALLEL
!$    CALL FACTOR(NTHREADS,LNTHREADS)
      CALL FACTOR(NPU,LNPU)
      CALL FACTOR8(N,IP)
      DO 10 I=1,3
!$      IF (IP(I) .GE. LNPU(I)*2+LNTHREADS(I)) THEN
!$        LNZ(I)=MAX0(LNPU(I)+LNTHREADS(I),(IP(I)+1)/3)
!$      ELSE
...
!$OMP PARALLEL PRIVATE(C,D)
      CALL PZFFT1D0(A,A,A,A,B,B,B,C,C,D,WX,WY,WZ,W,W(NY*(NZ/NPU)+1),
     1              NX,NY,NZ,ICOMM,NPU)
!$OMP END PARALLEL
C
      IF (IOPT .EQ. -1 .OR. IOPT .EQ. 1 .OR. IOPT .EQ. 2) THEN
        CALL MPI_ALLTOALL(B,NN/NPU,MPI_DOUBLE_COMPLEX,
