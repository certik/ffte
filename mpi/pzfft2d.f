...
C
      ...
      COMPLEX*16 WX(NDA2/3*2+NP),WY(NDA2/3*2+NP)
      DATA NX0,NY0/0,0/
      SAVE NX0,NY0,WX,WY
C
      IF (NX .NE. NX0) THEN
...
!$OMP END PARALLEL
      ELSE
!$OMP PARALLEL PRIVATE(C,D)
        CALL PZFFTR2D0(A,A,B,B,C,C,D,WX,WY,NX,NY,NPU)
!$OMP END PARALLEL
      END IF
      RETURN
      END
...
!$OMP DO
  ...
  260 CONTINUE
      RETURN
      END
      SUBROUTINE PZFFTR2D0(A,AX,B,BX,CX,CY,D,WX,WY,NX,NY,NPU)
      IMPLICIT REAL*8 (A-H,O-Z)
      INCLUDE 'param.h'
      COMPLEX*16 A(NX/NPU,*),AX(NX/NPU,NY/NPU,*)
      COMPLEX*16 B(NX,*),BX(NX/NPU,NY/NPU,*)
...
!$OMP DO
   ...
   40   CONTINUE
        DO 70 K=1,NPU
          DO 60 J=1,NNY
            DO 50 I=II,MIN0(II+NBLK-1,NNX)
              BX(I,J,K)=CY(K+(J-1)*NPU,I-II+1)*DN
   50       CONTINUE
   60     CONTINUE
   70   CONTINUE
   80 CONTINUE
...
!$OMP DO
  ...
  120   CONTINUE
        DO 130 J=JJ,MIN0(JJ+NBLK-1,NNY)
          CALL FFT23458R(CX(1,J-JJ+1),D,WX,NX,LNX)
  130   CONTINUE
        DO 150 J=JJ,MIN0(JJ+NBLK-1,NNY)
          DO 140 I=1,NX
            B(I,J)=CX(I,J-JJ+1)
