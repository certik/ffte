...
C
      ...
      NPUZ=(2**((LNPU(1)+1)/2)*(3**((LNPU(2)+1)/2))
     1    *(5**((LNPU(3)+1)/2)))
      NPUY=NPU/NPUZ
C
      ICOLOR=ME/NPUY
      CALL MPI_COMM_SPLIT(MPI_COMM_WORLD,ICOLOR,0,ICOMMY,IERR)
      ICOLOR=MOD(ME,NPUY)
      CALL MPI_COMM_SPLIT(MPI_COMM_WORLD,ICOLOR,0,ICOMMZ,IERR)
C
      IF (ME .EQ. 0) THEN
        WRITE(6,*) ' NX,NY,NZ ='
        READ(5,*) NX,NY,NZ
